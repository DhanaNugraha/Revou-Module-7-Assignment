...
def test_register_account(
    ...
    account = db.session.execute(db.select(AccountsModel).filter_by(id=1)).scalar_one()
...
def test_register_account_missing_field(
...
def test_register_account_repo_error(
    client, mock_admin_token_data, mock_account_data, users_data_inject
):
...
def test_update_account_details(
    client,
    mock_admin_token_data,
    mock_account_id,
    mock_update_account_data,
    users_data_inject,
    account_data_inject,
    db,
):
    response = client.put(
        f"/accounts/{mock_account_id}",
        headers=mock_admin_token_data,
        json=mock_update_account_data,
    )
...
def test_update_account_details_missing_field(
    client,
    mock_admin_token_data,
    mock_account_id,
    mock_update_account_data,
...
def test_update_account_details_repo_error(
    client,
    mock_admin_token_data,
    mock_account_id,
    mock_update_account_data,
...
def test_delete_account(
    client, mock_admin_token_data, mock_account_id, users_data_inject, account_data_inject
):
    response = client.delete(f"/accounts/{mock_account_id}", headers=mock_admin_token_data)
...
def test_get_account_details(
    client, mock_token_data, mock_account_id, users_data_inject, account_data_inject
):
...
def test_get_user_accounts(
    client, mock_token_data, users_data_inject, account_data_inject
):
